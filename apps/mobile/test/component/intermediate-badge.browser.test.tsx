 ...
 * flagging it as an intermediate occurrence. Real
 ...
 * and the word "Intermediate" lives only in the a11y label and the showLabel
...
describe("IntermediateBadge", () => {
  ...
  it("labels the mark as intermediate training for screen readers", () => {
    const screen = render(<IntermediateBadge isIntermediate />);
    expect(
      screen.getByTestId("intermediate-badge").getAttribute("aria-label"),
    ).toBe("Intermediate trening");
  });

  it("expands to glyph + 'Intermediate' when showLabel is set", () => {
    ...
    expect(badge.textContent).not.toContain("Intermediate trening");
  });
