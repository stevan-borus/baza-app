...
const ALLOWED_KEYS: ConsentDocumentKey[] = ["tos", "privacy", "eula", "waiver_adult", "waiver_minor"];
...
export default function LegalDocumentScreen() {
  ...
  const isValidKey = typeof key === "string" && (ALLOWED_KEYS as string[]).includes(key);
  ...
  return (
    <ScreenContainerRaw title={t("legal.viewerTitle")} headerVariant="detail">
      {!docKey ? (
        <Text className="text-danger px-6 py-6">{t("legal.loadError")}</Text>
      ) : docQuery.isLoading ? (
