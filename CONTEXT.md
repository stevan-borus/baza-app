...
Only a **late** single-Booking cancellation (per `shouldApplyLateCancelPenalty`) reaches operators: it generates a NotificationLog row for **all admins** and for **the Session's assigned Trainer**, and triggers an Expo push. An **early** cancellation notifies nobody — no NotificationLog row, no push. A client cancelling days ahead is routine, and the row-per-cancel fan-out read as spam in the operator feed; a late cancel is the one the studio has to react to. If a Trainer is also an Admin, they receive only the Trainer-flavored notification. The waitlist auto-promotion notification to the promoted Client is unrelated and unchanged. For **Bulk reservation cancel** the fan-out collapses (see that entry).
