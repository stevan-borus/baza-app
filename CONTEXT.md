...
**Intermediate** (zahtevno):
An optional Admin-set binary marking on a single Session telling Clients that occurrence is a demanding one — the middle tier the studio actually communicates to clients, worded "Zahtevno" ("Challenging") after the studio rejected both "napredno" and "srednji nivo". Unmarked (the default) is the norm. Strictly per-occurrence: marking one occurrence of a recurring series never touches its siblings. Admins set or clear it at any time, including after Clients have booked. Purely advisory and display-only: it gates nothing (no booking restrictions, no filtering) and setting/clearing it notifies no one. Rendered to every role as a wordless brand-green ★ text glyph riding the session's time/meta line; the booking sheet doubles as the legend, expanding the mark to "★ Zahtevno". The word otherwise survives only in the a11y label and the admin edit switch.
