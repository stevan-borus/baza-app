# Privacy Policy

**Version:** 1
**Effective date:** [LEGAL REVIEW: fill in date]

> Translation note: In case of any discrepancy between the Serbian and English versions, the Serbian version prevails.

## 1. About the Privacy Policy

1.1. This Privacy Policy is issued by "DANICA PIPER PR PILATES STUDIO BAZA NOVI SAD", a sole proprietor in the fitness club industry with registered seat at Futoška 33, Novi Sad, Serbia, registration number 68350247, tax ID 115417505 (hereinafter: "BAZA"). In this respect BAZA acts as the controller of the processing of your personal data.

1.2. The Privacy Policy applies only to natural persons, i.e. their personal data. Personal data are all data through which we can, directly or indirectly, identify you.

1.3. Please read this Privacy Policy. If you do not agree with our practices, please do not provide us with your personal data.

1.4. For any questions or requests regarding the processing or protection of your personal data, please contact us at bazapilates@gmail.com.

## 2. What is covered by the Privacy Policy?

2.1. The Privacy Policy covers the ways we process your personal data when you interact with us, e.g. when you:

- use our mobile application;
- visit our website;
- use our services;
- supply your goods or services;
- communicate with us in any other way or via any other channel.

2.2. Processing of personal data is any action or set of actions, performed in automated or non-automated form on personal data or sets of personal data, such as collection, recording, classification, grouping or structuring, storage, adaptation or alteration, disclosure, access, use, disclosure by transmission or making available, reproduction, dissemination or otherwise making available, comparison, restriction, erasure or destruction.

## 3. Which of your personal data do we process?

3.1. We process the personal data you have given us, whether at the initial contact, in subsequent communications (e.g. by telephone call, e-mail, etc.), or arising from our business cooperation.

3.2. We process the following personal data we have collected from you:

- **Your identification data** — first and last name, date of birth;
- **Your contact data** — e-mail address, telephone number;
- **additional data necessary for business purposes**, such as data needed for invoicing/payment.

3.3. We also process personal data related to your use of products and/or services, which is collected automatically.

3.4. We process the following personal data we have collected automatically: name of the website from which the visit was made, time and duration of the visit, unique device identifier, IP address, device type.

3.5. In accordance with the law, we will not process data related to:

- racial or ethnic origin;
- political opinions;
- religious or philosophical beliefs;
- trade union membership;
- genetic functions;
- sexual life;
- criminal convictions or related security measures;
- biometric data;
- genetic data.

### 3.6. Special categories of personal data

In certain cases we may process data relating to the health condition of the user, where the user voluntarily provides such data through the Application or in communication with the Studio. This data may include information about:

- current and past injuries;
- physical limitations;
- health conditions relevant to performing training;
- pregnancy or post-partum recovery;
- other information the user chooses to share for the purpose of adapting training.

The stated data is processed solely in order to adapt training to the user's individual needs; to protect the user's health and safety during exercise; and to provide adequate support from the instructor. Such data constitutes a special category of personal data under applicable personal data protection regulations and is processed exclusively on the basis of your explicit consent. The user is not obliged to provide the stated data, but in that case the Studio may not be able to fully adapt the training to the user's individual needs. Whenever there is a need to process any of the stated personal data, your explicit consent will always be required.

## 4. Why do we process your personal data?

4.1. We process your personal data because you are a user of our services or are interested in becoming one (e.g. you have sent us certain inquiries via the website).

4.2. We process your personal data for the following purposes:

- **Conclusion and execution of the contract**
  We process your personal data that we need in order to process, accept and fulfil an inquiry, create your user account, manage reservations, provide the services that are the subject of the contract, and receive payment for the service we provide.
- **Fulfilment of our legal duties**
  We process your personal data in order to fulfil our legal obligations, e.g. disclosure of data to state institutions or supervisory bodies in connection with reporting obligations and meeting requests from state authorities or other public bodies. In this case, the basis for processing your personal data is the fulfilment of our legal duties.
- **Information about requests and services**
  We process your personal data in order to inform you about the status of a particular request, the performance of a service or similar matters, all for the purpose of keeping you fully informed. In this case, the basis for processing your personal data is our legitimate interest.
- **Other cases**
  To protect our legitimate interests as a business entity (e.g. where this is necessary for conducting court and other proceedings or where this is necessary to ensure an adequate level of protection). Also, certain data is collected in order to improve your experience in our mobile application/website. In this case, the basis for processing your personal data is our legitimate interest.

## 5. Who has access to your personal data?

5.1. We treat your personal data as confidential and protect it accordingly in line with applicable legal regulations and best practice.

5.2. Your personal data will be processed by us, while third parties have the right to access and process your personal data only in the situations described below:

- Legal entities with whom we cooperate in business and which assist us in our regular operations (WhyNot Solutions). In that case, those legal entities process your personal data exclusively for our purposes;
- Persons with whom we cooperate in business, where we assess this is necessary to protect certain of our legitimate interests. E.g. tax advisors, accountants, lawyers and other advisors. In that case, those persons process your personal data exclusively for our purposes;
- Competent authorities supervising the legality of our operations and conduct, as well as other legal entities where this is necessary for the fulfilment of a legal duty of ours, e.g. an auditor. In that case, those legal entities process your personal data for the purpose assigned to them by law;
- Other parties in connection with business transactions such as the sale of the company or part of the company, reorganisation, merger, joint venture or any other form of disposal of our company, assets or stock (including bankruptcy or similar proceedings).

5.3. Third parties are restricted in their ability to use your personal data for any purposes other than those stated, and are obliged to protect and process your personal data in accordance with statutory, regulatory and contractual obligations.

## 6. Is my personal data transferred to other countries?

6.1. Your personal data is processed and stored on servers located in the European Union (EU), in accordance with the purpose set out in Section 3. Such transfer is permitted given that the condition of an adequate level of protection is met, meaning that the level of personal data protection is equivalent to the level guaranteed by the Law on the Protection of Personal Data.

6.2. For the purpose of providing and maintaining the Application, personal data may also be processed by trusted technical and business partners (e.g. cloud infrastructure providers, IT support or other technical service providers) which may have their seat or servers in the EU/EEA or other countries. In the case of transfer of data outside the EU/EEA, such transfer will be carried out solely with the application of appropriate safeguards in accordance with applicable personal data protection regulations, such as standard contractual clauses or other recognised protection mechanisms. We take reasonable technical, organisational and legal measures to ensure an adequate level of protection of personal data regardless of where it is processed.

6.3. The condition from the two preceding articles is met given that, by Decision of the Government of the Republic of Serbia of 01.08.2019, ___country___ was determined to be on the list of countries, parts of their territories or one or more sectors of certain activities in those countries and international organisations in which an adequate level of personal data protection is deemed to be ensured.

## 7. How do we protect your personal data?

7.1. The protection of your personal data is extremely important to us. Some of the protection measures we apply are the following:

- Application of modern methods of protection and access control to data sources containing personal data;
- Supervision over all resources used to process personal data.

7.2. The purpose of these measures is to prevent the risk of destruction, loss, alteration, unauthorised disclosure of, or access to, your personal data.

7.3. We also require the use of appropriate protection measures, in relation to your personal data, from third persons that have the right to access and process your personal data, as stated in Article 5.2.

## 8. For how long do we keep your personal data?

8.1. We keep your personal data for the entire duration of the contract we have concluded with you. After termination of the contract, we delete your data after an additional period of 11 years from the termination of the contract (the 10-year limitation period, plus one year for deletion).

8.2. Personal data we process on the basis of your consent we keep for as long as we have your consent. In the event consent is withdrawn, we delete the data as soon as possible.

8.3. We define the above retention periods for your personal data guided by the following criteria:

- the period during which we need your personal data in order to be able to provide our services to you;
- whether there is a statutory, contractual or similar obligation to retain your personal data.

## 9. Which regulations apply?

The protection of your personal data is regulated by the Law on the Protection of Personal Data ("Official Gazette of the Republic of Serbia", no. 87/2018).

## 10. Your rights

10.1. If you decide to exercise one or more of the rights listed below, the Company has the right to verify your identity, for the purpose of protecting personal data.

10.2. You exercise your rights free of charge. Exceptionally, if your request is manifestly unfounded or excessive (e.g. you are asking for personal data which you yourself already have), and particularly if the same request is repeated frequently (e.g. less than 6 months has passed since the last request), the Company has the right to charge necessary costs of acting on your request or to refuse to act on your request.

10.3. **Access to your personal data:**
You have the right to ask us for information on whether we process your personal data, access to that data, as well as information about the processing in accordance with Article 26 of the Law. Please send questions and requests, in writing and signed, to the address Futoška 33, 21000 Novi Sad. We will respond to your request immediately, and no later than within one month, unless the exceptional complexity of a particular case requires an extension of the said period.

10.4. **Correction of inaccurate data, completion, erasure, restriction of use of personal data:**
You have the right to request the correction of your inaccurate personal data, as well as the right to complete, delete and restrict the use of your personal data. Please send questions and requests, in writing and signed, to the address Futoška 33, 21000 Novi Sad. We will respond to your request immediately, and no later than within one month, unless the exceptional complexity of an individual case requires an extension of the said period.

10.5. **Portability of personal data:**
You have the right to receive and request the transfer of your personal data. Please send questions and requests, in writing and signed, to the address Futoška 33, 21000 Novi Sad. We will respond to your request immediately, and no later than within one month, unless the exceptional complexity of an individual case requires an extension of the said period.

10.6. **Right to withdraw consent and right to be forgotten:**
You have the right to withdraw your consent for the processing of personal data, as well as to request the permanent deletion of your personal data which we have processed on the basis of your consent. Please send questions and requests, in writing and signed, to the address Futoška 33, 21000 Novi Sad. We will respond to your request immediately, and no later than within one month, unless the exceptional complexity of a particular case requires an extension of the said period.

10.7. **Objection to the processing or handling of your personal data:**
You have the right to object to the processing of your personal data, as well as generally to the way we handle your personal data. Please send your request by e-mail to the address Futoška 33, 21000 Novi Sad, with "Objection against processing" in the subject of the message and stating the reason for the objection and your request in the message itself.

10.8. **Right to complain to the Commissioner for Information of Public Importance and Personal Data Protection:**
At any time you have the right to lodge a complaint with the competent authority for personal data protection — the Commissioner for Information of Public Importance and Personal Data Protection (https://www.poverenik.rs/).
